...
void FastQCModule::parse(const std::vector<MatchedFile>& files) {
    ...
    if (!samples_.empty()) {
        ...
        add_section("Per Sequence GC Content", "gc_content", "Distribution of GC content across sequences");
        add_section("Sequence Length Distribution", "seq_length", "Distribution of sequence lengths");
    }
}
...
void FastQCModule::add_basic_stats() {
    ...
    for (const auto& [sample_id, sample] : samples_) {
        data[sample_id] = {{"total_sequences", sample.total_sequences},
    ...
    headers = {
        {"total_sequences",
         {"Total Sequences", "Total number of sequences", "Blues", "{:,d}", "", "", false, 1000000, 0}},
        {"percent_gc", {"% GC", "GC content percentage", "Greens", "{:.1f}", "%", "", false, 100, 0}},
        {"avg_sequence_length",
         {"Avg Length", "Average sequence length", "Oranges", "{:,d}", " bp", "", false, 1000, 0}},
        {"percent_duplicates",
         {"% Duplicates", "Percentage of duplicate sequences", "Reds", "{:.1f}", "%", "", false, 100, 0}},
