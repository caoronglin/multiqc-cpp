...
void SamtoolsModule::parse(const std::vector<MatchedFile>& files) {
    ...
    if (!samples_.empty()) {
        ...
        headers = {
            ...
            {"percent_properly_paired", {"% Properly Paired", "Percentage of properly paired reads", "Oranges", "{:.1f}", "%", "", false, 100, 0}}
        };
...
bool SamtoolsModule::parse_flagstat(const std::string& filepath, const std::string& sample_name) {
    ...
    spdlog::debug("Parsed flagstat for {}: {} reads, {:.1f}% mapped", 
                  sample_name, data.total_reads, data.percent_mapped);
