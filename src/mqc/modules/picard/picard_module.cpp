...
void PicardModule::parse(const std::vector<MatchedFile>& files) {
    ...
    for (const auto& file : files) {
        if (file.fn.find("mark_duplicates") != std::string::npos || file.fn.find("metrics") != std::string::npos) {
    ...
    if (!samples_.empty()) {
        ...
        for (const auto& [sample_id, sample] : samples_) {
            data[sample_id] = {{"read_pairs", sample.read_pairs},
        ...
        headers = {
            ...
            {"percent_duplicates",
             {"% Duplicates", "Percentage of duplicates", "Reds", "{:.1f}", "%", "", false, 100, 0}},
            {"estimated_library_size",
             {"Est. Library", "Estimated library size", "Purples", "{:,d}", "", "", false, 1000000000, 0}},
            {"median_insert_size",
             {"Median Insert", "Median insert size", "Oranges", "{:.0f}", " bp", "", false, 1000, 0}},
    ...
    while (std::getline(ifs, line)) {
        ...
        if (fields.size() >= 8) {
            try {
                ...
                spdlog::debug("Parsed Picard MarkDuplicates for {}: {} pairs, {:.1f}% duplicates", sample_name,
                              data.read_pairs, data.percent_duplicates);
                return true;
    ...
    while (std::getline(ifs, line)) {
        ...
        if (fields.size() >= 6) {
            try {
                ...
                spdlog::debug("Parsed insert size for {}: median={:.0f}, mean={:.1f}", sample_name,
                              data.median_insert_size, data.mean_insert_size);
                return true;
            } catch (const std::exception& e) {
