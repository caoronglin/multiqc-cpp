...
FastpModule::FastpModule()
    : BaseModule("fastp", "fastp", "Ultra-fast all-in-one FASTQ preprocessor") {
}
...
bool FastpModule::parse_fastp_json(const std::string& filepath, const std::string& sample_name) {
    try {
        ...
        spdlog::debug("Parsed fastp for {}: {} reads, {:.1f}% passed, Q30={:.1f}%",
                     sample_name, data.total_reads, data.percent_passed, data.q30_rate);
...
void FastpModule::add_filtering_stats() {
    ...
    headers = {
        ...
        {"passed_reads", {"Passed Reads", "Number of reads passing filters", "Greens", "{:,d}", "", "", false, 100000000, 0}},
        {"percent_passed", {"% Passed", "Percentage of reads passing filters", "Greens", "{:.1f}", "%", "", false, 100, 0}},
