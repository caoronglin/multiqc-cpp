...
int main(int argc, char* argv[]) {
    ...
    if (!args.filename.empty())
        overrides["filename"] = args.filename;
    if (!args.title.empty())
        overrides["title"] = args.title;
    if (!args.template_name.empty())
        overrides["template"] = args.template_name;
    if (args.force)
    ...
    spdlog::info("Found {} files in {} directories", search_results.total_files, search_results.total_dirs);
