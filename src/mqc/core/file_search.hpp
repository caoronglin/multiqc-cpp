...
struct SearchOptions {
    ...
    SearchOptions() {
        ignore_dirs = {".git", ".svn", ".hg", "node_modules", "__pycache__", ".cache"};
    }
};
...
public:
     ...
     * @return 合并的搜索结果
     */
    SearchResults search_parallel(const std::vector<fs::path>& dirs, 
...
private:
    ...
    bool match_contents(const fs::path& filepath, const std::string& pattern, 
    ...
    // 并行遍历
    std::vector<std::future<SearchResults>> traverse_parallel(
    ...
    // 文件匹配
    std::optional<MatchedFile> match_file(const fs::path& filepath,
