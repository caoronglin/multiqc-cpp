...
FileSearcher::FileSearcher(SearchOptions options) : options_(std::move(options)) {}
...
void FileSearcher::add_pattern(const std::string& module_id, const SearchPattern& pattern) {
    ...
    spdlog::debug("Added pattern for module {}: {}", module_id, pattern.filename_pattern.value_or("*"));
}
...
FileSearcher::SearchResults FileSearcher::search(const fs::path& dir, ProgressCallback progress) {
    ...
    spdlog::info("Search completed in {}ms: {} files, {} dirs, {} matches", results.search_time.count(),
                 stats_.files_scanned, stats_.dirs_traversed, stats_.matches_found);

    return results;
}

void FileSearcher::traverse_directory(const fs::path& dir, SearchResults& results, ProgressCallback progress) {
    // 使用递归目录遍历
    std::error_code ec;
...
std::optional<MatchedFile> FileSearcher::match_file(const fs::path& filepath, const fs::path& root_dir) {
    ...
    for (const auto& [module_id, module_patterns] : patterns_) {
        for (const auto& pattern : module_patterns) {
            ...
            if (pattern.contents_pattern.has_value()) {
                ...
                if (!match_contents(filepath, pattern.contents_pattern.value(), pattern.max_lines, matched_content)) {
                    continue;
                }
...
bool FileSearcher::match_contents(const fs::path& filepath, const std::string& pattern, std::optional<int> max_lines,
...
FileSearcher::SearchResults FileSearcher::search_parallel(const std::vector<fs::path>& dirs,
...
FileSearcher::SearchResults FileSearcher::merge_results(const std::vector<SearchResults>& results) {
    ...
    for (const auto& result : results) {
        ...
        for (const auto& [module_id, files] : result.files_by_module) {
            merged.files_by_module[module_id].insert(merged.files_by_module[module_id].end(), files.begin(),
                                                     files.end());
        }
    }
