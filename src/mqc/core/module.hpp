...
class BaseModule
{
  public:
     ...
     */
    void add_section(const std::string& name, const std::string& anchor, const std::string& description,
     ...
     */
    void general_stats_addcols(const nlohmann::json& data, const std::map<std::string, GeneralStatsColumn>& headers);
...
#define REGISTER_MODULE(ID, CLASS)                                                       \
    static bool CLASS##_registered = []() {                                              \
        ModuleRegistry::register_module(ID, []() { return std::make_unique<CLASS>(); }); \
        return true;                                                                     \
    }();
