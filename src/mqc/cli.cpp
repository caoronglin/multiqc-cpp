...
void CommandLineArgs::setup_main_command() {
    ...
    // 帮助
    app.add_flag("-h,--help", "Print this help message")
       ->group("")
       ->trigger_on_parse()
       ->callback([&]() {
           std::cout << app.help();
...
void CommandLineArgs::setup_fastp_command() {
    ...
    fastp_cmd->add_option("-i,--in1", fastp.input1, "Input R1 file (required)")
              ->required()
              ->check(CLI::ExistingFile);
    
    fastp_cmd->add_option("-I,--in2", fastp.input2, "Input R2 file (PE data)")
              ->check(CLI::ExistingFile);
